...
```bash
sudo AURUM_DNS_GATEWAY=10.0.0.1 AURUM_DNS_CIDRS=10.0.0.0/24 bash install-dns.sh
```
...
- ставит команды `aurum-dns-status`, `aurum-dns-test`, `aurum-dns-restart`;
...
```bash
sudo unbound-checkconf /etc/unbound/unbound.conf.d/aurum-vpn.conf
...
- удалит `/etc/unbound/unbound.conf.d/aurum-vpn.conf`;
- удалит команды `aurum-dns-*`;
