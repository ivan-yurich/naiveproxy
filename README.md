...
[![Version](https://img.shields.io/badge/version-5.5.10-D4A017?style=for-the-badge)](https://github.com/ivan-yurich/naiveproxy/releases)
...
## Что нового в v5.5.10

Версия `5.5.10` исправляет запуск Unbound DNS при включении доступа для VPN-клиентов: теперь Unbound не занимает `0.0.0.0:53`, а слушает только локальные и конкретные IPv4-адреса сервера.

| Направление | Что добавлено |
|---|---|
| Recursive DNS | Unbound может работать как собственный recursive resolver без Cloudflare/Google upstream |
| DNSSEC | Добавлен trust anchor и проверка `dnssec-failed.org` в статусе |
| DNS для VPN | Можно открыть `:53` только для заданных VPN CIDR, без open resolver |
| Bind fix | VPN DNS больше не конфликтует с `systemd-resolved` на `127.0.0.53:53` |
| Режимы | `recursive + adblock`, `recursive only`, `forward DoT + adblock`, `forward DoT only` |
| Unbound plugin | Старый DNS-блокировщик оформлен как отдельный модуль: `unbound`, `unbound-install`, `unbound-status` |
...
Unbound умеет два основных режима:

- `recursive` — собственный DNS-резолвер без Google/Cloudflare, с DNSSEC и кэшем;
- `forward DoT` — старый режим через Cloudflare/Google DNS-over-TLS.

Для VPN-клиентов DNS открывается только по указанным CIDR. Скрипт не разрешает делать open resolver для `0.0.0.0/0`, а Unbound слушает конкретные IPv4-адреса сервера/VPN-интерфейсов вместо `0.0.0.0:53`.
