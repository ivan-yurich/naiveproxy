...
|---|---|
| Бренд | **Yurich** |
| Клиентское приложение/выдача | **Yurich Connect** |
| SSH/Telegram панель | **Yurich Panel** |
| Серверное ядро | **Yurich Core** |
| DNS-модуль | **Yurich DNS** |
| Windows-направление | **Yurich Desktop** |
| Android-направление | **Yurich Mobile** |
| Персональная подписка | **Yurich ID** |
| Сайт и личные страницы | **Yurich Cloud** |

`Yurich Net` зарезервирован как название для будущего сетевого/инфраструктурного направления. Технические термины `NaiveProxy`, `Xray`, `Hysteria 2`, `Caddy` и `Unbound` остаются в документации там, где речь идёт именно о протоколах и компонентах.
...
Версия `5.6.2` проводит ребрендинг проекта в экосистему **Yurich**: панель, ядро, DNS, клиентская выдача, подписки и сайт получили единый нейминг. Также сохраняются инструменты v5.6.x: выбор языка SSH-панели, self-update с SHA256, pin версий ключевых компонентов, health-check, safe apply, encrypted backup, export/import и сроки подписок.

| Направление | Что добавлено |
|---|---|
| Brand refresh | `Yurich`, `Yurich Panel`, `Yurich Core`, `Yurich Connect`, `Yurich DNS`, `Yurich ID`, `Yurich Cloud` |
...
| Yurich Mobile subscriptions | Имена URI в подписке получают метку срока, чтобы профиль проще отличался в клиентах |
...
| Yurich DNS CLI | `aurum-dns-status` и uninstall читают env только при владельце `root` |
...
| Yurich DNS | Отдельный production-ready проект `aurum-dns/` с install/uninstall/examples/scripts |
...
- Yurich DNS на Unbound для приватного DNS внутри VPN;
- SSH hardening;
- Fail2Ban;
...
Yurich DNS поднимает собственный recursive Unbound resolver для сервера и VPN-клиентов:
...
Для VPN-клиентов DNS открывается только по указанным CIDR. Если gateway IP, например `10.0.0.1`, ещё не назначен интерфейсу сервера, скрипт предложит создать безопасный локальный gateway `10.0.0.1/32` на `lo`. Полный sing-box Yurich Mobile VPN/TUN конфиг на странице подписки автоматически получит Yurich DNS.
...
- добавлена архитектура бренда: `Yurich Connect`, `Yurich Panel`, `Yurich Core`, `Yurich DNS`, `Yurich Desktop`, `Yurich Mobile`, `Yurich ID`, `Yurich Cloud`;
- интерфейс, Telegram-сообщения, страницы подписки, DNS-модуль и README переведены на новый нейминг;
...
- добавлена команда `health` для проверки Caddy, Yurich DNS, Telegram bot service, WARP, Xray и Hysteria;
...
- полный sing-box Yurich Mobile VPN/TUN конфиг теперь автоматически включает Yurich DNS через `tcp://10.0.0.1:53`;
- персональная страница подписки показывает готовый sing-box TUN конфиг с Yurich DNS.

### v5.5.11

- удалена DNS-блокировка рекламы из основного скрипта;
- добавлен безопасный Yurich DNS режим на Unbound для VPN-клиентов;
...
- Yurich Cloud: [ivan-it.net](https://ivan-it.net)
