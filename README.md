...
[Мультисервер](MULTISERVER_GUIDE_RU.md) ·
[HAProxy](HAPROXY_GUIDE_RU.md) ·
[FAQ](#faq) ·
[Changelog](#changelog)
...
|---|---|
| GitHub-ready release | Подготовлена публичная версия без личных доменов, IP, токенов, Telegram ID и локальных путей |
| Security policy | Добавлены `SECURITY.md`, `.gitignore`, `.gitattributes` и правила безопасной публикации |
| Protocol benchmark | Улучшена логика `protocol-benchmark`: median/p95/slow_count и защита от ложного SLOW по одному замеру |
| Protocol validation | Уточнена проверка `links.txt`, `hiddify.txt`, `nekobox.txt`, `v2rayng.txt` и реальных клиентских URI |
| Public placeholders | Контакты, donate, сайт, канал и поддержка заменены на демонстрационные значения |
| HAProxy guide | Добавлена расширенная статья [HAPROXY_GUIDE_RU.md](HAPROXY_GUIDE_RU.md) по TCP/SNI routing на `443/tcp` |
...
[Yurich Panel 5.6.x: большой обзор скрипта](ARTICLE_RU.md)
...
- Синхронизированы `yurich-panel.sh` и `naiveproxy.sh` перед публикацией;
- обновлены `yurich-panel.sh.sha256` и `naiveproxy.sh.sha256`;
- README приведён к актуальной версии релиза;
- закреплён PingTunnel на проверяемый релиз вместо `latest`;
- добавлена SHA256-проверка PingTunnel для Linux amd64/arm64;
- `pingtunnel-config` теперь скрывает секреты по умолчанию;
- `ssh-rescue` получил автоотключение аварийного режима через TTL;
- подтверждена проверка Bash-синтаксиса для основных скриптов и DNS-модулей.

### v5.6.49

- Подготовлена GitHub-safe версия проекта для публичного репозитория;
- удалены/заменены личные домены, IP-адреса, Telegram ID, токены, приватные ссылки, локальные пути и коммерческие QR/контакты;
- добавлены `SECURITY.md`, `.gitignore`, `.gitattributes` и правила безопасной публикации;
- расширен [MULTISERVER_GUIDE_RU.md](MULTISERVER_GUIDE_RU.md): rollout без простоя, HAProxy/SNI mux, WARP, обновление node, проверка профилей, удаление node из подписок;
- улучшена документация по `protocol-validate`, `protocol-benchmark` и `protocol-benchmark-monitor`;
- slow-детект в benchmark описан как устойчивый сценарий: median/p95/slow_count вместо вывода по одному случайному замеру;
- зафиксирована публичная модель протоколов: HTTPS = Yurich Proxy/Naive-compatible, Turbo = Hysteria 2, Reality = VLESS Reality;
- скрипты оставлены совместимыми: `yurich-panel.sh` основной, `naiveproxy.sh` сохранён как alias.

### v5.6.34

- Добавлено меню `29) [NODES] Multi-server management`;
- добавлен реестр серверов `/etc/naiveproxy/nodes.conf`;
- добавлены команды `nodes`, `nodes-list`, `nodes-add`, `nodes-test`, `nodes-deploy`, `nodes-sync`, `nodes-subscriptions`, `nodes-remove`;
- добавлена синхронизация пользователей и metadata на node-серверы с backup перед применением;
- добавлены удалённые helper-команды `hysteria-sync` и `xray-rebuild`;
- страницы подписки и `links.txt` теперь могут содержать ссылки на дополнительные включённые node-домены.

### v5.6.12

- На страницу подписки добавлены ссылки:
  - Yurich Connect Android: `https://github.com/ivan-yurich/Yurich-Connect-Android/releases`;
  - Yurich Connect Windows: `https://github.com/ivan-yurich/yurich-connect-windows/releases`;
- добавлены контакты проекта: Telegram, VK и `support@example.com`;
- карточки Windows/Android теперь ведут на официальные release-страницы клиента.
...
- добавлен пункт Xray-меню `REALITY target presets / test`;
- добавлена CLI-команда `xray-target`;
- текущий REALITY target теперь виден в Xray-меню.

### v5.6.10

- Добавлен рабочий VLESS XHTTP TLS standalone на `8448/tcp`;
- XHTTP больше не зависит от `Xray fallback hub` на `443`;
- страницы подписки и `links.txt` получают XHTTP-ссылку автоматически;
- UFW, status, diagnose и uninstall знают про порт `8448/tcp`;
- fallback XHTTP на `443` сохранён как дополнительный вариант, если включён fallback hub.

### v5.6.9

- Исправлено “зависание” установки после строки `Сгенерирован пароль`;
- prompt срока пользователя теперь выводится напрямую в терминал и не попадает внутрь переменной;
- при ошибке срока сообщение выводится в stderr, а не захватывается command substitution;
- старый `v5.6.8` можно продолжить вручную: нажать `Enter` на пустом месте, это выберет срок `12` месяцев.

### v5.6.8

- SSH-панель переведена на terminal-safe ASCII labels вместо emoji;
- главное меню больше не показывает `�` в VNC/serial-консолях без emoji-шрифтов;
- диагностика использует `[OK]`, `[WARN]`, `[FAIL]` вместо emoji-статусов;
- DNS и donate терминальные экраны очищены от emoji;
...
- Донат: [Donation page](https://example.com/donate)
